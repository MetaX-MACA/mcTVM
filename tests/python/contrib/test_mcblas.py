...
from tvm.contrib import mcblaslt
from tvm.contrib import graph_executor
import tvm.testing
from tvm.relay.op.contrib import get_pattern_table
from tvm.relay.op.contrib.mcblas import partition_for_mcblas
...
def verify_matmul_add(in_dtype, out_dtype, rtol=1e-5):
    ...
    C = mcblas.matmul(A, B, dtype=out_dtype)
    s = te.create_schedule(C.op)

    def verify(target="maca"):
        if not tvm.get_global_func("tvm.contrib.mcblas.matmul", True):
...
def verify_matmul_add_igemm(in_dtype, out_dtype, rtol=1e-5):
    ...
    # C has MCBLASLT_ORDER_COL32 layout, thus a different shape
    C = mcblaslt.matmul(A, B, False, True, m, N_out, dtype=out_dtype)
    s = te.create_schedule(C.op)

    def verify(target="maca"):
        if not tvm.get_global_func("tvm.contrib.mcblaslt.matmul", True):
            print("skip because extern function is not available")
            return
        dev = tvm.maca(0)
        f = tvm.build(s, [A, B, C], target)
        ...
        # Transform a to become MCBLASLT_ORDER_COL4_4R2_8C layout
        ...
        # Transform b to become MCBLASLT_ORDER_COL32 layout
        b_new = np.vstack(
        ...
        f(a, b, c)
        # Transform output c from layout MCBLASLT_ORDER_COL32 to row major layout
        c_out = c.numpy()
...
def verify_batch_matmul(Ashape, Bshape, Cshape, in_dtype, out_dtype, rtol=1e-5):
    ...
    C = mcblas.batch_matmul(A, B, dtype=out_dtype)
    s = te.create_schedule(C.op)

    dev = tvm.maca(0)
    f = tvm.build(s, [A, B, C], "maca")
...
def _verify_mcblas_relay(expr):
    np.random.seed(42)

    mod = tvm.IRModule.from_expr(expr)
    mod = relay.transform.InferType()(mod)
    func = mod["main"]
    mcblas_mod = partition_for_mcblas(mod)
    assert len(mcblas_mod.get_global_vars()) == 2
    ...
    maca_config = (tvm.target.maca(), tvm.maca(), mcblas_mod)
...
def test_relay_mcblas_matmul(n, m, k, in_dtype, out_dtype, transpose_a, transpose_b):
    unsupported_configs = [
...
def test_relay_mcblas_dense(n, m, k, in_dtype, out_dtype):
    unsupported_configs = [
...
def test_relay_mcblas_batch_matmul(
