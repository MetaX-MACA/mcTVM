...
from tvm.contrib.mxcc import have_fp16
from tvm.contrib import graph_executor
import numpy as np
import tvm.topi.testing
import tvm.testing
from tvm.relay.op.contrib.mcdnn import partition_for_mcdnn


requires_mcdnn = pytest.mark.skipif(
    tvm.get_global_func("tvm.contrib.mcdnn.conv2d.forward", True) is None,
    reason="McDNN is not enabled",
)
...
def verify_conv2d(data_dtype, conv_dtype, tensor_format=0, groups=1):
    ...
    if data_dtype == "float16" and not have_fp16(tvm.maca(0).compute_version):
    ...
    # validation
    dev = tvm.maca(0)
    f = tvm.build(s, [X, W, Y], "maca --host=llvm", name="conv2d")
...
def verify_conv3d(data_dtype, conv_dtype, tensor_format=0, groups=1):
    ...
    # validation
    dev = tvm.maca(0)
    f = tvm.build(s, [X, W, Y], target="maca --host=llvm", name="conv3d")
    x_np = np.random.uniform(-1, 1, xshape).astype(data_dtype)
    w_np = np.random.uniform(-1, 1, wshape).astype(data_dtype)
    y_np = np.zeros(yshape).astype(data_dtype)
    x = tvm.nd.array(x_np, dev)
    ...
    f(x, w, y)
    tvm.testing.assert_allclose(y.numpy(), c_np, atol=3e-4, rtol=1e-4)


@tvm.testing.requires_gpu
@requires_mcdnn
@pytest.mark.skip("fail, Max absolute difference: 0.00356224")
def test_conv3d():
...
def verify_softmax(shape, axis, dtype="float32", log_softmax=False):
    mcdnn_op = mcdnn.log_softmax if log_softmax else mcdnn.softmax
    testing_op = (
    ...
    f = tvm.build(s, [A, B], target="maca --host=llvm", name="softmax")
    f(a, b)
...
def verify_conv2d_backward_data(data_dtype, conv_dtype, tensor_format=0, tol=1e-5):
    ...
    dev = tvm.maca(0)
    f = tvm.build(s, [dy, w, dx], "maca --host=llvm", name="conv2d_backward_data")
...
def test_conv2d_backward_data():
    # FIXME: Max absolute difference: 0.00397015
...
def verify_conv2d_backward_filter(data_dtype, conv_dtype, tensor_format=0, tol=1e-5):
    ...
    dev = tvm.maca(0)
    f = tvm.build(s, [dy, x, dw], "maca --host=llvm", name="conv2d_backward_filter")
...
@tvm.testing.requires_gpu
@requires_mcdnn
@pytest.mark.skip("fail: Max absolute difference: 0.01865146")
...
def _verify_mcdnn_relay(expr):
    np.random.seed(42)

    mod = tvm.IRModule.from_expr(expr)
    mod = relay.transform.InferType()(mod)
    func = mod["main"]
    mcdnn_mod = partition_for_mcdnn(mod)
    assert len(mcdnn_mod.get_global_vars()) == 2
    ...
    maca_config = (tvm.target.maca(), tvm.maca(), mcdnn_mod)
    cpu_config = (tvm.target.Target("llvm"), tvm.cpu(), mod)
    outputs = []
    for target, dev, test_mod in [maca_config, cpu_config]:
...
def test_relay_mcdnn_log_softmax(shape, axis, dtype):
    x = tvm.relay.var("x", tvm.relay.TensorType(shape, dtype))
    log_softmax = relay.op.nn.log_softmax(x, axis=axis)
    _verify_mcdnn_relay(log_softmax)
...
def test_relay_mcdnn_conv2d(n, h, w, ci, co, kh, kw, strides, dilation, padding, groups, dtype):
...
def test_relay_mcdnn_conv2d_bias_act(
