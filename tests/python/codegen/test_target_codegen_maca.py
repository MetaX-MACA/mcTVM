...
def test_maca_vectorize_add():
    ...
    check_maca("float32", 64, 2)
    check_maca("float32", 64, 3)
    check_maca("float32", 64, 4)
    check_maca("int8", 64, 2)
    check_maca("int8", 64, 3)
    check_maca("int8", 64, 4)
    check_maca("uint8", 64, 2)
    check_maca("uint8", 64, 3)
    check_maca("uint8", 64, 4)
    check_maca("float16", 64, 2)
    check_maca("float16", 64, 4)
    check_maca("float16", 64, 6)
    check_maca("float16", 64, 8)


@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_bf16_vectorize_add():
    num_thread = 8
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
@pytest.mark.skip("TODO: int8 and __dp4a is not supported by MACA C500")
def test_maca_multiply_add():
    num_thread = 8
...
def test_maca_vectorize_load():
    ...
    check_maca("int8", 64, 3)
    check_maca("int8", 64, 4)
    check_maca("int8", 64, 8)
    check_maca("int8", 64, 16)


@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_make_int8():
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_reduction_binding():
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_const_float_to_half():
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_floordiv_with_vectorization():
    with tvm.target.maca():
        # B[i] = A[floordiv(i, k)]
        n = 256
        k = 37
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_floormod_with_vectorization():
    with tvm.target.maca():
        # B[i] = A[floormod(i, k)]
        n = 256
        k = 37
...
def test_vectorized_casts():
    ...
    def skip(t0, t1):
        if t0 == t1:
            return True
        # MACA does support cast between {u}int8 and fp16.
...
def sched(A, B):
    ...
    return tvm.compile(sch.mod, target="maca")
...
def test_maca_vectorize_load_permute_pad():
    ...
    check_maca("int8", 64, 16, 3, 2)
    check_maca("uint8", 64, 16, 3, 2)
    check_maca("int8", 64, 16, 3, 4)
    check_maca("uint8", 64, 16, 3, 4)
    check_maca("int32", 64, 16, 3, 4)
    check_maca("float16", 64, 16, 3, 4)
    check_maca("float32", 64, 16, 3, 4)
...
@tvm.testing.requires_gpu
@tvm.testing.requires_maca
def test_maca_thread_sync_inside_condition():
    @T.prim_func
