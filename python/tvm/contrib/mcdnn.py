...
# under the License.
# ruff: noqa: F821
"""External function interface to mcDNN library."""
...
# algos can be read from mcdnn.h
_FWD_ALGOS = [
...
def exists():
    """
    Checks whether the local machine can use mcDNN.

    Returns
    -------
        exists: bool

            True if mcDNN support is enabled and a mcDNN-capable GPU
            exists.  Otherwise, False.
    """
    func = tvm.get_global_func("tvm.contrib.mcdnn.exists", allow_missing=True)
    if func is None:
        return False
...
def algo_to_index(algo_type, algo_name):
    ...
    algo_name : str
        Algorithm name as defined in mcDNN. For example:

        * fwd: ``MCDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM``, etc.
        * bwd_filter: ``MCDNN_CONVOLUTION_BWD_FILTER_ALGO_0``, etc.
        * bwd_data: ``MCDNN_CONVOLUTION_BWD_DATA_ALGO_0``, etc.
...
def conv_output_shape(
    ...
    if tensor_format == 0:
        ...
        x_chan = x_shape[-1]
        w_chan_input = w_shape[-1]
        assert len(x_shape) == 4, "mcDNN layout NHWC is only well-defined for 4d tensors"
        x_shape = x_shape[1:-1]
        w_shape = w_shape[1:-1]
        ...
        raise ValueError(f"Unknown mcDNN tensor format: '{tensor_format}'")
...
def conv_dgrad_shape(
    ...
    if tensor_format == 0:
        ...
        raise ValueError(f"Unsupported mcDNN tensor format: '{tensor_format}'")
...
def _conv_find_algo(
    ...
    """
    Common function to choose the best mcdnn convolution algorithm for the given input
...
def conv_forward_find_algo(
    ...
    -------
    algo: int
        algo chosen by MCDNN
    """
    return _conv_find_algo(
        "tvm.contrib.mcdnn.conv.forward_find_algo",
        tensor_format,
        pad,
        stride,
        dilation,
...
def conv_backward_data_find_algo(
    ...
    """
    return _conv_find_algo(
        "tvm.contrib.mcdnn.conv.backward_data_find_algo",
        tensor_format,
        pad,
        stride,
        dilation,
...
def conv_backward_filter_find_algo(
    ...
    """
    return _conv_find_algo(
        "tvm.contrib.mcdnn.conv.backward_filter_find_algo",
        tensor_format,
        pad,
        stride,
        dilation,
...
def conv_forward(
    ...
    """Create an extern op that compute 2D or 3D convolution with mcDNN
    ...
    algo: int
        ...
        if algo == -1, the best algo will be chosen by MCDNN
    conv_dtype: str
        convolution type
    groups: int
    ...
    if isinstance(x.shape[0], tvm.tirx.expr.IntImm):
        ...
        if algo == -1:
            # For now if we try to call `mcdnnFindConvolutionForwardAlgorithm` when
            # using INT8 data type, mcDNN will crash down.
            # On the other hand, mcDNN only support IMPLICIT_PRECOMP_GEMM at NHWC format
        ...
        oshape[0] = x.shape[0]
        # This picks MCDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM
    ...
    if dims == 4:
        return te.extern(
            ...
            lambda ins, outs: tvm.tirx.call_packed(
                "tvm.contrib.mcdnn.conv2d.forward",
                conv_mode,
                tensor_format,
                algo,
                pad[0],
    ...
    return te.extern(
        ...
        [x, w],
        lambda ins, outs: tvm.tirx.call_packed(
            "tvm.contrib.mcdnn.conv3d.forward",
            conv_mode,
            tensor_format,
            algo,
            pad[0],
...
def conv_backward_data(
    ...
    """Create a mcDNN extern op that computes the gradient of 2D convolution with respect to data.
    ...
    assert isinstance(dy.shape[0], tvm.tirx.expr.IntImm), (
        "Dynamic batch is not supported for mcdnn conv2d backwad data yet."
    )
    ...
    if exists():
        # When mcdnn exists, find the backward data algo
    ...
    return te.extern(
        ...
        [dy, w],
        lambda ins, outs: tvm.tirx.call_packed(
            "tvm.contrib.mcdnn.conv2d.backward_data",
            conv_mode,
            tensor_format,
            algo,
            pad[0],
...
def conv_backward_filter(
    ...
    """Create a mcDNN extern op that computes the gradient of 2D convolution with respect to weight.
    ...
    assert isinstance(x.shape[0], tvm.tirx.expr.IntImm), (
        "Dynamic batch is not supported for mcdnn conv2d backwad filter yet."
    )
    ...
    return te.extern(
        ...
        [dy, x],
        lambda ins, outs: tvm.tirx.call_packed(
            "tvm.contrib.mcdnn.conv2d.backward_filter",
            conv_mode,
            tensor_format,
            algo,
            pad[0],
...
def softmax(x, axis=-1):
    ...
    return te.extern(
        ...
        [x],
        lambda ins, outs: tvm.tirx.call_packed(
            "tvm.contrib.mcdnn.softmax.forward", ins[0], outs[0], axis
        ),
        name="y",
    )
...
def log_softmax(x, axis=-1):
    """Compute log_softmax using mcDNN
    ...
    return te.extern(
        ...
        [x],
        lambda ins, outs: tvm.tirx.call_packed(
            "tvm.contrib.mcdnn.log_softmax.forward", ins[0], outs[0], axis
        ),
        name="y",
    )
