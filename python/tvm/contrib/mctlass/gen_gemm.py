...
"""GEMM kernel generator and profiler for MCTLASS."""
...
def create_gemm_operator_with_epilogue(
    ...
    """
    Instantiate a mctlass kernel from the given configuration,
    ...
    if "residual" in op_type:
        if "hardswish" in op_type:
            activation = "mctlass::epilogue::thread::HardSwish"
        elif "silu" in op_type:
            activation = "mctlass::epilogue::thread::SiLu"
        elif "sigmoid" in op_type:
            activation = "mctlass::epilogue::thread::Sigmoid"
        elif "gelu" in op_type:
            activation = "mctlass::epilogue::thread::GELU"
        elif "relu" in op_type:
            activation = "mctlass::epilogue::thread::ReLu"
        else:
            activation = "mctlass::epilogue::thread::Identity"

        binary_op = "mctlass::multiplies" if "residual_multiply" in op_type else "mctlass::plus"
        unary_op = (
            "mctlass::epilogue::thread::ReLu"
            if op_type.endswith("relu")
            else "mctlass::epilogue::thread::Identity"
        )
        residual_block_info = {
...
DEFAULT_KERNELS = {
    75: {
        ("float16", "float16"): "mctlass_tensorop_h1688gemm_128x64_32x2_tn_align1",
        ("float16", "float32"): "mctlass_tensorop_s1688gemm_f16_64x64_32x2_tn_align1",
    },
    ...
    80: {
        ...
        ("float32", "float32"): (
            "mctlass_tensorop_s1688gemm_128x64_32x3_tn_align1",
            "mctlass_tensorop_s1688gemm_64x64_16x3_tn_align1",
        ),
    },
}


class MctlassGemmProfiler:
    """Profile all candidate kernels and select the best one."""

    def __init__(self, sm, mctlass_path, binary_path):
        assert sm in GENERATOR_FUNC_TABLE and sm in DEFAULT_KERNELS, f"sm{sm} not supported yet."
        self.engine = ProfilerEngine(sm, mctlass_path, binary_path)
        self.sm = sm
        self.cache_path = os.path.join(binary_path, "mctlass_gemm_cache.pickle")
