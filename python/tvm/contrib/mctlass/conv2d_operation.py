...
"""Generator for MCTLASS Conv2D kernels."""
...
class Conv2dOperation:
    ...
    def procedural_name(self):
        ...
        if self.stride_support == StrideSupport.Unity:
            configuration_name = (
                "mctlass_${opcode_class}_${extended_name}_${threadblock}"
...
class EmitConv2dInstance:
    """Responsible for emitting a MCTLASS template definition."""
    ...
    ${epilogue_functor}<
      ...
      ${element_accumulator},
      ${element_epilogue},
      mctlass::epilogue::thread::ScaleType::NoBetaScaling
    >"""
  ...
  typename mctlass::conv::kernel::DefaultConv2d${conv_kind_name}${conv_kernel_postfix}<
    ${element_a},
    ${layout_a},
    ${element_b},
    ${layout_b},
    ...
    ${opcode_class},
    ${arch},
    mctlass::gemm::GemmShape<${threadblock_shape_m}, ${threadblock_shape_n}, ${threadblock_shape_k}>,
    mctlass::gemm::GemmShape<${warp_shape_m}, ${warp_shape_n}, ${warp_shape_k} >,
    mctlass::gemm::GemmShape<${instruction_shape_m}, ${instruction_shape_n}, ${instruction_shape_k}>,
    ${epilogue},
    ${swizzling_functor}, // mctlass::gemm::threadblock::GemmSplitKIdentityThreadblockSwizzle<>,
    ${stages},
    ${math_operator},
    ${iterator_algorithm},
    ${stride_support},
...
using ReductionOp = mctlass::reduction::thread::ReduceAdd<
    ${element_accumulator},
    ${element_accumulator},
...
using ReductionDevice = mctlass::reduction::device::ReduceSplitK<ReductionKernel>;
...
"""
    ...
    def emit(
        ...
        values = {
            ...
            "arch": f"mctlass::arch::Sm{operation.arch}",
...
def instantiate_conv2d_template(attrs):
    """Return MCTLASS host code for conv2d based on a template and the provided attribute map."""
    template = """
    ${mctlass_op_def}

  using Conv2d = mctlass::conv::device::ImplicitGemmConvolution<${mctlass_op_name}>;
  ...
  mctlass::conv::Conv2dProblemSize problem_size(N, H, W, C, K, R, S, P, Q, pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w, mctlass::conv::Mode::kCrossCorrelation, split_k_slices);
  const mctlass::conv::SplitKMode split_k_mode = mctlass::conv::SplitKMode::${split_k_mode};
  ...
  using mctlass::layout::TensorNHWC;
  auto activation_shape = TensorNHWC::packed(mctlass::make_Coord(N, H, W, C));
  auto weight_shape = TensorNHWC::packed(mctlass::make_Coord(K, R, S, C));
  auto output_shape = TensorNHWC::packed(mctlass::make_Coord(N, P, Q, K));
  ${residual_shape_decl}
  ...
  mctlass::TensorRef<ElementOutput, TensorNHWC> tensor_c{static_cast<ElementOutput*>(${tensor_c}), ${tensor_c_layout}};
  mctlass::TensorRef<ElementOutput, TensorNHWC> tensor_d{static_cast<ElementOutput*>(ptr_out), layout_D};
  ...
  mctlass::Status status = conv2d_op.can_implement(arguments);
  TVM_FFI_ICHECK(status == mctlass::Status::kSuccess);
  ${split_k_reset}
  status = conv2d_op.initialize(arguments, workspace.get());
  TVM_FFI_ICHECK(status == mctlass::Status::kSuccess);
  ${split_k_update}

  mcStream_t stream = static_cast<mcStream_t>(TVMFFIEnvGetStream(kDLMACA, ${data_arg}->device.device_id));
...
"""
  ...
  const static mctlass::conv::Operator kConvolutionalOperator = Conv2d::kConvolutionalOperator;
...
"""
    ...
    use_split_k = "splitk" in attrs["mctlass_op_name"]
    ...
    no_bias_scaling = op_type not in [
        "mctlass.conv2d_bias_sigmoid",
        "mctlass.conv2d_bias_silu",
        "mctlass.conv2d_bias_hardswish",
    ]
    ...
    if has_residual_block:
        ...
        aux_map["tensor_c_layout"] = "mctlass::layout::TensorNHWC::Stride(0)"
    else:
        aux_map["bias_decl"] = ""
        aux_map["residual_decl"] = ""
    ...
    if is_wgrad:
        ...
        if has_residual_block:
            ...
            shape_str = f"mctlass::make_Coord({res_shape[0]}, {res_shape[1]}, {res_shape[2]}, K)"
            aux_map["residual_shape_decl"] = (
