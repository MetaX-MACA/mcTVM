...
# ruff: noqa: F821
"""Driver for partitioning and building a Relax module for MCTLASS offload."""
...
from tvm.support.mxcc import find_maca_path
from tvm.topi.utils import get_const_tuple

from .gen_conv2d import MctlassConv2DProfiler
from .gen_gemm import MctlassGemmProfiler
...
def has_mctlass():
    """Returns true if the MCTLASS custom codegen is available"""
    return tvm.get_global_func("relax.ext.mctlass", True) is not None


def _get_mctlass_path():
    mctlass_path = os.path.join(find_maca_path(), "include/mctlass")
    return mctlass_path


def _get_mctlass_compile_options(threads, use_fast_math=False):
    kwargs = {}
    kwargs["cc"] = "mxcc"
    kwargs["options"] = [
        "-c",
        "-DMCTLASS_ENABLE_TENSOR_CORE_MMA=1",
        "-DNDEBUG",
        "-Xcompiler=-fPIC",
    ...
    if use_fast_math:
        kwargs["options"].append("-DMCTLASS_USE_TANH_FOR_SIGMOID")
    return kwargs
...
def select_gemm_kernel(
    ...
    """Run MCTLASS profiler to select the best kernel, or return the default one for dynamic
    workloads."""
    if any(isinstance(s, tvm.tirx.Any) for s in [MM, KK, NN]):
        out = mctlass_profiler.get_default(
            op_type, out_dtype, arg0_dtype, arg1_dtype, use_3xtf32, batched=batched
        )
        name, mctlass_op_def = out["name"], out["opdef"]
        logger.info("Picked the default kernel %s", name)
    else:
        name, mctlass_op_def, _ = mctlass_profiler.profile(
            op_type,
            MM,
            NN,
            KK,
...
def handle_batch_matmul(
    ...
    name, mctlass_op_def = select_gemm_kernel(
        mctlass_profiler,
        op_type,
        MM,
        KK,
    ...
    return {
        ...
        "mctlass_op_def": mctlass_op_def,
        "mctlass_op_name": name,
        "lda": "K",
        "ldb": "K",
        "ldc": "N",
    }
...
def handle_conv2d(
    ...
    return {"mctlass_op_def": mctlass_op_def, "mctlass_op_name": name}


def num_mctlass_partitions(mod):
    return sum([(1 if "mctlass" in var.name_hint else 0) for var in mod.get_global_vars()])


def tune_mctlass_kernels(
    mod,
    use_3xtf32=True,
    split_k_slices=[1],
    ...
    """Given a module partitioned for MCTLASS offloading, profile each workload to select which
    kernels to emit.

    Parameters
    ----------
    mod : IRModule
        The IRModule with mctlass partitions.
    ...
    -------
    mod : IRModule
        The updated module annotated with mctlass profiling information.

    num_mctlass_partition : int
        The number of partitioned functions created for MCTLASS.
    """
    gemm_profiler = MctlassGemmProfiler(_get_mctlass_path(), tmp_dir)
    conv2d_profiler = MctlassConv2DProfiler(_get_mctlass_path(), tmp_dir)
    num_mctlass_partition = 0
    for var in mod.get_global_vars():
        fun_name = var.name_hint
        func = mod[fun_name]
        if "mctlass" in fun_name:
            num_mctlass_partition += 1
            new_func = tune_mctlass_function(
                func,
                use_3xtf32,
                split_k_slices,
                profile_all_alignments,
    ...
    return mod, num_mctlass_partition
...
def is_shape_valid_for_mctlass_matmul(
    lhs_shape: Sequence[tvm.ir.Expr],
    rhs_shape: Sequence[tvm.ir.Expr],
) -> bool:
    """
    Check whether the shape of inputs can be handled by MCTLASS GEMM.

    The stride-based batch matmul in MCTLASS cannot handle cases that some of
...
class MctlassRelaxFunctionAnnotator(relax.PyExprMutator):
    """A Relax function mutator that tunes and annotates MCTLASS composite functions
    ...
    def __init__(
        self,
        mod,
        conv2d_profiler: MctlassConv2DProfiler,
        gemm_profiler: MctlassGemmProfiler,
        options,
    ):
        super().__init__(mod)
    ...
    def handle_conv2d(self, f, op_type):
        ...
        attrs = {
            ...
            "padding": padding,
            "dilation": dilation,
            "mctlass_op_name": op_name,
            "mctlass_op_def": op_def,
        }
    ...
    def handle_matmul(self, f, op_type):
        ...
        if not is_shape_valid_for_mctlass_matmul(lhs_shape, rhs_shape):
        ...
        if lhs_batches == 1 and rhs_batches == 1:
            ...
            batch_attrs = {
                ...
                # they must be equal. This is checked by is_shape_valid_for_mctlass_matmul.
    ...
    def handle_norm(self, f, op_type):
        ...
        attrs["data_type"] = {"float32": "float", "float16": "mctlass::half_t"}[str(dtype)]
...
@register_global_func("contrib.mctlass.tune_relax_function")
def profile_relax_function(functions, options):
    """Tune and annotate MCTLASS composite functions with shape, dtype and generated templates."""
    tmp_dir = options.get("tmp_dir", "./tmp")
    sm = options.get("sm", 80)
    conv2d_profiler = MctlassConv2DProfiler(sm, _get_mctlass_path(), tmp_dir)
    gemm_profiler = MctlassGemmProfiler(sm, _get_mctlass_path(), tmp_dir)

    annotated_functions = []

    for f in functions:
        annotator = MctlassRelaxFunctionAnnotator(
...
@register_global_func("contrib.mctlass.compile")
def compile_mctlass_module(c_source_module, options):
    """Compile all MCTLASS kernels in the given C-source module.

    Parameters
    ----------
    c_source_module: runtime.Module
        A C-source module containing MCTLASS kernels.

    options: dict
        Compilation options. Currently recognizes
          "sm": The target architecture (compute capability), for example 75 or 80 (default: 80)
          "threads": The number of threads to use in MXCC parallel compilation (default:
          use all logical cores)
          "use_fast_math": Whether or not to use faster but approximate arithmetic in some
          MCTLASS epilogues (default: False)

    Returns
    -------
    rt_mod : runtime.Module
        A runtime module where all mctlass kernels have been compiled.
    """
    tmp_dir = options.get("tmp_dir", "./tmp")
    defaults = {"threads": -1, "use_fast_math": False}
    ...
    compile_options = _get_mctlass_compile_options(**compile_config)
    lib_path = os.path.join(tmp_dir, "mctlass.o")
    logger.info("Compiling generated MCTLASS code")
...
def finalize_modules(lib, lib_path="compile.so", tmp_dir="./tmp"):
    ...
    for use by the graph or AOT executors. This method is not specific to MCTLASS, however it does
