...
"""Generator for MCTLASS GEMM kernels."""
...
class GemmOperation:
    ...
    def procedural_name(self):
        ...
        return substitute_template(
            "mctlass_${opcode_class}_${extended_name}_${threadblock}_${layout}_align${alignment}",
            {
...
class EmitGemmInstance:
  ...
  using Operation_${operation_name} = mctlass::gemm::device::${kernel_name}<
    ${element_a}, ${layout_a},
    ${element_b}, ${layout_b},
    ${element_c}, ${layout_c},
    ${element_accumulator},
...
def instantiate_gemm_template(attrs):
    """Return MCTLASS host code for GEMM based on a template and the provided attribute map."""
  ...
  typename ${kernel}::Arguments arguments{
   ...
   ${split_k_slices_or_batch}
    ...
    # See mctlass/gemm/kernel/gemm_with_fused_epilogue.h
    argument_template_residual = """
  typename ${kernel}::Arguments arguments{
    mctlass::gemm::GemmUniversalMode::${gemm_universal_mode},
    problem_size,
  ...
  using ${kernel} = Operation_${mctlass_op_name};
  int M = ${M};
  int N = ${N};
  int K = ${K};
  ...
  ${kernel} gemm_op;
  mctlass::Status status = gemm_op.can_implement(arguments);
  TVM_FFI_ICHECK(status == mctlass::Status::kSuccess);
  status = gemm_op.initialize(arguments, workspace.get());
  TVM_FFI_ICHECK(status == mctlass::Status::kSuccess);

  mcStream_t stream = static_cast<mcStream_t>(TVMFFIEnvGetStream(kDLMACA, ${lhs_arg}->device.device_id));
...
def emit_fp16A_intB_matmul(attrs):
    """Return MCTLASS host code for fp16 A and int4 or int8 B GEMM."""
    if attrs["group_size"] > 0:
        attrs["quant_op"] = "mctlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_ONLY"
    else:
        attrs["quant_op"] = "mctlass::WeightOnlyQuantOp::PER_COLUMN_SCALE_ONLY"
        attrs["group_size"] = "k"
  ...
  mcStream_t stream = static_cast<mcStream_t>(
    TVMFFIEnvGetStream(kDLMACA, ${A_arg}->device.device_id));
    """,
        attrs,
    )
    ...
    template = """
  ${template_common}
  gemm_fp16_int_bias_act<${weight_dtype}, QuantOp>(static_cast<mctlass::half_t*>(${A_arg}->data),
                static_cast<${weight_dtype}*>(${B_arg}->data),
                static_cast<mctlass::half_t*>(${scales_arg}->data),
                ${bias},
                static_cast<mctlass::half_t*>(out0->data),
                "${activation}",
...
"""
  ...
  gemm_fp16_int_bias_act_residual<${weight_dtype}, QuantOp>(
                static_cast<mctlass::half_t*>(${A_arg}->data),
                static_cast<${weight_dtype}*>(${B_arg}->data),
                static_cast<mctlass::half_t*>(${scales_arg}->data),
                ${bias},
                static_cast<mctlass::half_t*>(${residual_arg}->data),
                static_cast<mctlass::half_t*>(out0->data),
...
"""
    ...
    if "residual_arg" in attrs:
        if "bias_arg" in attrs:
            bias = "static_cast<mctlass::half_t*>(${bias_arg}->data)"
        else:
            bias = "nullptr"
    ...
    if "bias_arg" in attrs:
        template = substitute_template(
            template, {"bias": "static_cast<mctlass::half_t*>(${bias_arg}->data)"}
        )
    else:
