...
# ruff: noqa: F821
"""Common functions and classes for MCTLASS GEMM and Conv2d geneator."""
...
logger = logging.getLogger("mctlass")
    ...
    if arg0_dtype == "float16" and arg1_dtype == "float16":
        math_instructions = [
            MathInstruction(
                ...
                DataType.f16,
                DataType.f16,
                dtype_map[str(out_dtype)],
                dtype_map[str(accumlator_dtype)],
...
EPILOGUE_MAP = {
    "mctlass.dense": (EpilogueFunctor.LinearCombination, False),
    "mctlass.dense_bias": (EpilogueFunctor.LinearCombinationBias, True),
    "mctlass.dense_bias_relu": (EpilogueFunctor.LinearCombinationRelu, True),
    "mctlass.dense_bias_gelu_fp16": (EpilogueFunctor.LinearCombinationGelu, False),
    "mctlass.dense_bias_gelu_fp32": (EpilogueFunctor.LinearCombinationGelu, False),
    "mctlass.matmul": (EpilogueFunctor.LinearCombination, False),
    "mctlass.matmul_bias": (EpilogueFunctor.LinearCombinationBias, True),
    "mctlass.matmul_bias_relu": (EpilogueFunctor.LinearCombinationRelu, True),
    "mctlass.matmul_bias_gelu": (EpilogueFunctor.LinearCombinationGelu, False),
    "mctlass.matmul_transposed": (EpilogueFunctor.LinearCombination, False),
    "mctlass.matmul_transposed_bias": (EpilogueFunctor.LinearCombinationBias, True),
    "mctlass.matmul_transposed_bias_relu": (EpilogueFunctor.LinearCombinationRelu, True),
    "mctlass.matmul_transposed_bias_gelu": (EpilogueFunctor.LinearCombinationGelu, False),
    "mctlass.batch_matmul": (EpilogueFunctor.LinearCombination, False),
    "mctlass.conv2d_bias_hardswish": (EpilogueFunctor.LinearCombinationHardSwish, False),
    "mctlass.conv2d_bias_silu": (EpilogueFunctor.LinearCombinationSilu, False),
    "mctlass.conv2d_bias_sigmoid": (EpilogueFunctor.LinearCombinationSigmoid, False),
    "mctlass.conv2d_bias_relu": (EpilogueFunctor.LinearCombinationRelu, True),
    "mctlass.conv2d_bias": (EpilogueFunctor.LinearCombinationBias, True),
    "mctlass.conv2d": (EpilogueFunctor.LinearCombination, False),
    "mctlass.conv2d_transpose": (EpilogueFunctor.LinearCombination, False),
    "mctlass.conv2d_backward_weight": (EpilogueFunctor.LinearCombination, False),
}
...
class ProfilerEngine:
    ...
    def _compile(self, op):
        ...
        if os.path.exists(opath):
            ...
            "w", delete=False, prefix=self.binary_prefix, suffix=".maca"
        )
        fi.write(op["src"])
        fi.close()
...
@tvm_ffi.register_global_func("contrib.mctlass.instantiate_template")
def instantiate_template(func_name, annotations, func_args):
    """Return MCTLASS host code based on a template and the provided annotations.
    ...
    codegen_result : CodegenResult
        Generated MCTLASS host code and required header-file names.
    """
    attrs = {}

    for k in ["lda", "ldb", "ldc", "mctlass_op_def", "mctlass_op_name", "op_type"]:
        if k in annotations:
    ...
    if "relu" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_bias_relu.h")
    elif "gelu" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_gelu.h")
    elif "sigmoid" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_sigmoid.h")
    elif "silu" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_silu.h")
    elif "hardswish" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_hardswish.h")
    else:
        headers.append("mctlass/epilogue/thread/linear_combination.h")

    if "residual" in func_name:
        headers.append("mctlass/epilogue/thread/linear_combination_residual_block.h")

    def get_dim(shape_annot, var_name, axis_idx, batched_offset=0):
        if isinstance(shape_annot, (IntImm | int)):
    ...
    if "decode_matmul" in func_name:
        headers.append("mctlass_kernels/fpA_intB_gemm.h")
        ...
        if annotations["weight_nbit"] == 4:
            attrs["weight_dtype"] = "mctlass::uint4b_t"
        ...
        attrs["ElementInputA"] = DataTypeTag[dtype_map[str(annotations[f"arg{lhs_arg_idx}_dtype"])]]
        attrs["ElementInputB"] = DataTypeTag[dtype_map[str(annotations[f"arg{rhs_arg_idx}_dtype"])]]
        attrs["ElementOutput"] = DataTypeTag[dtype_map[str(annotations["ret_dtype"])]]
        ...
        if batched:
            headers.append("mctlass/gemm/device/gemm_batched.h")
            ...
            headers.append("mctlass/gemm/device/gemm.h")

        if "residual" in func_name:
            headers.append("mctlass/gemm/device/gemm_universal_with_broadcast.h")
        ...
        if "conv2d_transpose" in func_name:
            headers.append("mctlass/conv/kernel/default_conv2d_dgrad.h")
            activation_shape = output_shape
            output_shape = annotations["arg0_shape"]
        elif "backward" in func_name:
            headers.append("mctlass/conv/kernel/default_conv2d_wgrad.h")
            activation_shape = annotations["arg1_shape"]
            weight_shape = output_shape
            output_shape = annotations["arg0_shape"]
        elif "residual" in func_name:
            headers.append("mctlass/conv/kernel/default_conv2d_fprop_with_broadcast.h")
        else:
            headers.append("mctlass/conv/kernel/default_conv2d_fprop.h")

        headers.append("mctlass/conv/device/implicit_gemm_convolution.h")

        op_name = attrs["mctlass_op_name"]

        if "splitk" in op_name:
            headers += [
                "mctlass/reduction/device/reduce_split_k.h",
                "mctlass/reduction/thread/reduction_operators.h",
            ]
        ...
        use_flash = (
            ...
            # attention workloads. Otherwise, MCTLASS fMHA seems faster for causal attention
            # with a single query.
        ...
        if use_flash:
            ...
            assert not is_mqa, (
                "The number of query and KV heads need to be the same for MCTLASS fMHA."
            )
            ...
            assert attrs["scale"] > 0 or attrs["scale"] < 0, (
                "Mctlass may generate nan occasionally when scale == 0.0"
            )
            attrs["arch"] = "mctlass::arch::Sm{}".format(annotations["arch"])
            ...
            if "bias" in attrs:
                ...
                # To support negative scale in current Mctlass implementation,
        ...
        headers.append("mctlass/util/device_layernorm.h")
        headers.append("mctlass/layout/matrix.h")
        attrs = {"input": func_args[0], "gamma": func_args[1], "beta": func_args[2]}
        attrs.update(dict(annotations))

        if not isinstance(attrs["M"], IntImm):
        ...
        headers.append("mctlass/util/device_rmsnorm.h")
        headers.append("mctlass/layout/matrix.h")
