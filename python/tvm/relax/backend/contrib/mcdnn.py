...
"""Pattern table for mcDNN backend"""
import operator
...
def _is_supported_dtype(lhs_dtype, rhs_dtype):
    """Check if dtypes in the given workload are supported by mcDNN BYOC."""
...
def _is_supported_format(data_layout, kernel_layout):
    """Check if layouts in the given workload are supported by mcDNN BYOC."""
...
def _check_conv2d(context: PatternCheckContext) -> bool:
    ...
    # Check if the data types of input and weights are supported by mcDNN BYOC
...
def _check_stacked_attention(context: PatternCheckContext, layout: str) -> bool:
    """Check if the given stacked attention workload can be offloaded to mcDNN."""
    if has_leaking_intermediate_variables(context):
        return False
    if layout == "BS3NH":
...
register_patterns(
    [
        ...
        (
            "mcdnn.conv2d.nhwc_ohwi_bias_relu",
            *make_conv2d_pattern(
                with_bias=True,
...
def partition_for_mcdnn(mod):
    """
    Partition the input module into mcDNN-supported subgraphs.
    ...
    mod: tvm.IRModule
        ...
        offloaded to the mcDNN backend.
    """

    patterns = get_patterns_with_prefix("mcdnn")
...
class WorkspaceAnnotator(PyExprMutator):
    """Annotate a workspace requirement for each mcDNN-offloaded function."""
    ...
    def visit_function_(self, f):
        if "Composite" not in f.attrs:
            ...
            if "global_symbol" in f.attrs and "mcdnn" in f.attrs["global_symbol"]:
        ...
        if "attention" in f.attrs["Composite"] and "mcdnn" in f.attrs["Composite"]:
...
def annotate_workspace(mod, _):
    """Pass to annotate a workspace requirement for each mcDNN-offloaded function."""
