...
"""Pattern table for mcBLAS backend"""
import operator
...
def _is_supported_dtype(lhs_dtype, rhs_dtype, out_dtype):
    """Check if dtypes in the given workload are supported by mcBLAS BYOC."""
...
def _check_matmul(context: PatternCheckContext) -> bool:
    ...
    if lhs_dtype == "int8" and rhs_dtype == "int8":
        ...
        if not rhs_transposed:
            # mcBLAS FP8 operations require rhs being transposed
            return False

        # mcBLAS FP8 operations require all tensors being aligned to 16 bytes.
        if (
    ...
    if "bias" in context.annotated_expr:
        ...
        if not isinstance(bias_batches, (tvm.tir.expr.IntImm, int)) or int(bias_batches) > 1:
            # mcBLAS only supports bias vector
            return False
...
register_patterns(
    [
        ...
        (
            "mcblas.matmul_transposed_bias_relu",
            *make_matmul_pattern(
                with_bias=True,
                activation="relax.nn.relu",
                transposed_rhs=True,
        ...
        (
            "mcblas.matmul_transposed_bias_gelu",
            *make_matmul_pattern(
                with_bias=True,
                activation="relax.nn.gelu",
                transposed_rhs=True,
        ...
        (
            "mcblas.matmul_transposed_dequantize",
            *make_matmul_dequantize_pattern(transposed_rhs=True),
            _check_matmul,
        ),
        (
            "mcblas.matmul_transposed_multiply",
...
def partition_for_mcblas(mod, bind_constants=False):
    """
    Partition the input module into mcBLAS-supported subgraphs.
    ...
    mod: tvm.IRModule
        ...
        offloaded to the mcBLAS backend.
    """

    patterns = get_patterns_with_prefix("mcblas")
