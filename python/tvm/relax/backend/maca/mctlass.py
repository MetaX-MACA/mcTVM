...
"""Pattern table for MCTLASS backend"""
...
import tvm
from tvm.contrib.mctlass.build import is_shape_valid_for_mctlass_matmul
...
def _is_supported_dtype(lhs_dtype, rhs_dtype):
    """Check if dtypes in the given workload are supported by MCTLASS."""
    return (
...
def _check_residual(root_call: Call, context: PatternCheckContext) -> bool:
    if "residual" in context.annotated_expr:
        ...
        if _has_dependency(from_var=residual, to_var=root_var, var_usages=context.var_usages):
            # If residual depends on the result of the root call, this cannot be handled by mctlass.
            return False
...
def _check_conv2d(context: PatternCheckContext) -> bool:
    """Check if the given conv2d workload can be offloaded to MCTLASS."""
...
def _check_matmul(context: PatternCheckContext) -> bool:
    """Check if the given matmul workload can be offloaded to MCTLASS."""
    if has_leaking_intermediate_variables(context):
        return False
    ...
    return is_shape_valid_for_mctlass_matmul(lhs_shape, rhs_shape)
...
def matmul_patterns():
    """
    Returns a list of all matmul patterns in mctlass BYOC backend.
    """
    ...
    return [
        _matmul_pattern("mctlass.matmul"),
        _matmul_pattern("mctlass.matmul_bias"),
        _matmul_pattern("mctlass.matmul_bias_relu"),
        _matmul_pattern("mctlass.matmul_bias_gelu"),
        _matmul_pattern("mctlass.matmul_transposed"),
        _matmul_pattern("mctlass.matmul_transposed_bias"),
        _matmul_pattern("mctlass.matmul_transposed_bias_relu"),
        _matmul_pattern("mctlass.matmul_transposed_bias_gelu"),
    ]


def _check_decode_matmul(ctx):
    """Check if the given decode -> matmul workload can be offloaded to MCTLASS."""
    ...
    if (
        ...
        and packed_weight.args[0].global_symbol != "mctlass.ft_preprocess_weight"
    ):
        return False
...
def decode_matmul_patterns():
    ...
    return [
        _decode_matmul_pattern("mctlass.decode_matmul"),
        _decode_matmul_pattern("mctlass.decode_matmul_bias"),
        _decode_matmul_pattern("mctlass.decode_matmul_cast"),
        _decode_matmul_pattern("mctlass.decode_matmul_cast_bias"),
        _decode_matmul_pattern("mctlass.decode_matmul_bias_gelu"),
        _decode_matmul_pattern("mctlass.decode_matmul_cast_bias_gelu"),
    ]


def conv2d_patterns():
    """
    Returns a list of all conv2d patterns in mctlass BYOC backend.
    """
    ...
    return [
        _conv2d_pattern("mctlass.conv2d"),
        _conv2d_pattern("mctlass.conv2d_bias"),
        _conv2d_pattern("mctlass.conv2d_bias_relu"),
        _conv2d_pattern("mctlass.conv2d_bias_silu"),
    ]


def residual_block_patterns():
    """
    Returns a list of all residual block patterns in mctlass BYOC backend.
    """
    patterns = []
...
def layer_norm_pattern():
    """Create a layer norm pattern for MCTLASS."""
    return [
        (
            "mctlass.layer_norm",
            *make_layer_norm_pattern(),
...
class WorkspaceAnnotator(PyExprMutator):
    """Annotate a workspace requirement for each MCTLASS-offloaded function."""
    ...
    def visit_function_(self, f):
        if "Composite" not in f.attrs:
            ...
            if "global_symbol" in f.attrs and "mctlass" in f.attrs["global_symbol"]:
...
def annotate_workspace(mod, _):
    """Pass to annotate a workspace requirement for each MCTLASS-offloaded function."""
...
def partition_for_mctlass(mod, annotate_codegen=True):
    """
    Partition the input module into MCTLASS-supported subgraphs.
    ...
    mod: tvm.IRModule
        ...
        compiled by the MCTLASS backend.
    """
    for func_name, func in mod.functions_items():
        mod[func_name] = func

    patterns = get_patterns_with_prefix("mctlass")
