...
class MACAMatmulTensorization(GPUScheduleRule):
    """
    The schedule rule for float16 mma computation.
    func with attr 'dlight.do_not_tensorize' will not be tensorized.
    """
    ...
    def apply(  # pylint: disable=too-many-locals,missing-docstring
        ...
        from tvm.tir.tensor_intrin.maca import (  # pylint: disable=import-outside-toplevel
            get_wmma_intrin_group,
        )
...
class MACAMatmulInt8Tensorization(GPUScheduleRule):
    """
    The schedule rule for int8 mma computation.
    func with attr 'dlight.do_not_tensorize' will not be tensorized.
    """
...
class Matmul(GPUScheduleRule):
    ...
    def get_configs(self, target: Target) -> Config:
        ...
        if target.kind.name == "cuda" or target.kind.name == "rocm" or target.kind.name == "maca":
    ...
    def apply(  # pylint: disable=too-many-locals,missing-docstring
        ...
        if target.kind.name == "cuda" and check_sm_version(target.arch) >= 70:
            ...
            if apply_tensorization:
                ...
                tensorize_sch = None
                if in_dtype == "int8" and out_dtype == "int32":
                    tensorize_sch = MACAMatmulInt8Tensorization().apply(func, target, _)
                elif in_dtype == "float16" and out_dtype in ["float16", "float32"]:
                    tensorize_sch = MACAMatmulTensorization().apply(func, target, _)
                if tensorize_sch is not None:
                    return tensorize_sch
