...
def maca_mma_schedule(
    ...
    """Create a tensorized schedule for GEMM with MACA MMA intrinsics."""
    ...
    i, i_tc = sch.split(i, factors=[None, mma_m])
    j, j_tc = sch.split(j, factors=[None, mma_n])
    ...
    def fetch_to_shared(block, idx, ndim):
        ...
        _, f_1, f_2, f_3 = sch.split(
            fused,
            factors=[None, num_ty, warp_size, vector_size],
        )
        sch.bind(f_2, "threadIdx.x")
        sch.bind(f_1, "threadIdx.y")
        sch.vectorize(f_3)
    ...
    ii, jj = sch.get_loops(C_warp)[-2:]
    i0, ii = sch.split(ii, factors=[None, 16])
    j0, ji = sch.split(jj, factors=[None, 16])
    sch.reorder(i0, j0, ii, ji)

    sch.decompose_reduction(block_outer, sch.get_loops(block_outer)[3])
    block_init_c = sch.get_sblock("C_init")

    def tile_mma_fragment(block_read, height, width):
        i, j = sch.get_loops(block_read)[-2:]
        i0, i1 = sch.split(i, factors=[None, height])
        j0, j1 = sch.split(j, factors=[None, width])
        sch.reorder(i0, j0, i1, j1)
    ...
    loop_a = tile_mma_fragment(A_warp, 16, k_inner)

    if b_transposed:
        loop_b = tile_mma_fragment(B_warp, 16, k_inner)
    else:
        loop_b = tile_mma_fragment(B_warp, k_inner, 16)

    # logical tile -> lane/local register layout
    sch.transform_layout(A_warp, ("write", 0), index_map_A)
    sch.transform_layout(B_warp, ("write", 0), index_map_B)
    sch.transform_layout(C_warp, ("read", 0), index_map_C)

    sch.tensorize(loop_a, load_a_intrin)
    sch.tensorize(loop_b, load_b_intrin)

    mma_loop = sch.get_loops(block_inner)[-3]
    sch.tensorize(mma_loop, mma_intrin)
