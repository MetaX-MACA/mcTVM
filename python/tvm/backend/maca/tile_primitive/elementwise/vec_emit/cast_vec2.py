...
_DTYPE_X2_NAME = {
    "float32": "float2",
    "float16": "__half2",
    "bfloat16": "__maca_bfloat162",
}


def _intrinsic_name(src_dtype, dst_dtype):
    return f"tvm_builtin_maca_cast_{src_dtype}x2_{dst_dtype}x2"
...
def _intrinsic_source(src_dtype, dst_dtype):
    ...
    return (
        ...
        f"(void* dst, const void* src) {{\n"
        f"    *reinterpret_cast<{_DTYPE_X2_NAME[dst_dtype]}*>(dst) = "
        f"{intrinsic}(*reinterpret_cast<const {_DTYPE_X2_NAME[src_dtype]}*>(src));\n"
        "}\n"
    )
...
def _emit_cast_vec2(dst_buf, dst_lane_indices, src_args, extras) -> Expr:
    src_arg = src_args[0]
    # The applicability check guarantees a buffer source with a pair tail.
    ...
    return T.maca.func_call(
        _intrinsic_name(src_dtype, dst_dtype),
