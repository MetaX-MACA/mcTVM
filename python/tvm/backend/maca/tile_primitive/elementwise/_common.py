...
def layout_signature(layout):
    """Return a semantic layout signature split by thread/local/replica axes."""
    ...
    replica_sig = [
        (_axis_key(it.axis), it.extent, it.stride)
        for it in (getattr(layout, "replica", None) or [])
    ]
