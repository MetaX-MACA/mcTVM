...
def get_swizzle(layout) -> ComposeLayout | None:
    """Return a ComposeLayout that carries a non-trivial swizzle, if any."""
    if isinstance(layout, ComposeLayout) and int(layout.swizzle_len) > 0:
        return layout
    return None


def apply_swizzle(swizzle: ComposeLayout, offset):
    """Apply only ComposeLayout's swizzle to an already tile-mapped offset."""
    per_element = int(swizzle.per_element)
    swizzle_len = int(swizzle.swizzle_len)
    atom_len = int(swizzle.atom_len)
    chunk = _IntImm("int32", 1 << per_element)
    quotient = tvm.tirx.floordiv(offset, chunk)
    mask = (1 << swizzle_len) - 1

    if bool(swizzle.swizzle_inner):
        xor = tvm.tirx.shift_right(
            tvm.tirx.bitwise_and(quotient, _IntImm("int32", mask << atom_len)),
            _IntImm("int32", atom_len),
        )
    else:
        xor = tvm.tirx.shift_left(
            tvm.tirx.bitwise_and(quotient, _IntImm("int32", mask)),
            _IntImm("int32", atom_len),
        )

    return tvm.tirx.shift_left(
        tvm.tirx.bitwise_xor(quotient, xor), _IntImm("int32", per_element)
    ) + tvm.tirx.floormod(offset, chunk)
...
def emit_init(pattern: SwizzlePattern, s_off_resolved):
    """Emit at thread setup (call from inside the @T.prim_func body):
      ...
      1. ``base_off = apply_swizzle(swizzle, s_off_resolved)`` — runtime, per-thread,
         computed once.
    ...
    base_off = apply_swizzle(swizzle, s_off_resolved)
...
def emit_fallback_offset(swizzle: ComposeLayout, s_off_resolved, ds_k):
    """Slow but always-correct path: swizzle ``s_off + ds_k``
    ...
    """
    return apply_swizzle(swizzle, s_off_resolved + ds_k)
