...
dispatches (currently ``vec_auto_gmem_smem`` and ``ldgsts``).

``vec_auto_gmem_smem`` (sync ``Tx.copy`` global ↔ shared) and ``ldgsts`` (async
...
def _alignment_ok(vec_len: int, terms) -> bool:
    ...
    directly; PrimExpr / symbolic terms checked via ``sym.Analyzer``.
...
def align_layouts_gs(
    ...
    if isinstance(s_layout, ComposeLayout) and int(s_layout.swizzle_len) > 0:
