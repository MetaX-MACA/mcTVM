...
"""Tensorcore template for maca backend"""
import numpy as np
import tvm
from tvm import te
...
from ..cuda.tensor_intrin import intrin_wmma_load_matrix_A
from ..cuda.tensor_intrin import intrin_wmma_load_matrix_W
from ..cuda.tensor_intrin import intrin_wmma_store_matrix
from ..cuda.tensor_intrin import intrin_wmma_gemm


def nhwc_tensorcore_maca(cfg, Input, Filter, stride, padding, dilation, out_dtype):
...
def schedule_nhwc_tensorcore_maca(cfg, s, Conv):
...
@autotvm.register_topi_compute("conv2d_nhwc_tensorcore.maca")
def conv2d_nhwc_tensorcore(cfg, data, kernel, strides, padding, dilation, out_dtype):
    ...
    return nhwc_tensorcore_maca(cfg, data, kernel, strides, padding, dilation, out_dtype)


@autotvm.register_topi_schedule("conv2d_nhwc_tensorcore.maca")
def schedule_conv2d_nhwc_tensorcore(cfg, outs):
    ...
    def _callback(op):
        if "conv2d_nhwc_tensorcore" in op.tag:
            schedule_nhwc_tensorcore_maca(cfg, s, op.output(0))
