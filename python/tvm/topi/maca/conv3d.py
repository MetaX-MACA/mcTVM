...
@autotvm.register_topi_compute("conv3d_mcdnn.maca")
def conv3d_mcdnn(
    cfg, data, kernel, strides, padding, dilation, groups, layout="NCDHW", out_dtype="float32"
):
    """Conv3D operator for maca backend.
    ...
    assert groups == 1, "conv3d_mcdnn does not support groups"
...
@autotvm.register_topi_schedule("conv3d_mcdnn.maca")
def schedule_conv3d_mcdnn(_, outs):
    """TOPI schedule callback of conv3d for maca gpu
