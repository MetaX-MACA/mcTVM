...
def conv2d_hwnc_tensorcore(data, kernel, strides, padding, dilation, in_dtype, out_dtype="int32"):
    ...
    packed_out = hwnc_tensorcore_maca(data, kernel, strides, padding, dilation, out_dtype)
    return unpack_HWNCnc_to_hwnc(packed_out, out_dtype)


@autotvm.register_topi_compute("conv2d_HWNCnc_tensorcore.maca")
def hwnc_tensorcore_maca(cfg, Input, Filter, stride, padding, dilation, out_dtype="int32"):
...
def schedule_hwnc_tensorcore_maca(cfg, s, Conv):
...
@autotvm.register_topi_schedule("conv2d_HWNCnc_tensorcore.maca")
def schedule_conv2d_hwnc_tensorcore(cfg, outs):
    ...
    def _callback(op):
        if "conv2d_HWNCnc_tensorcore" in op.tag:
            schedule_hwnc_tensorcore_maca(cfg, s, op.output(0))
