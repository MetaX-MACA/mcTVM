...
/*!
 * \file src/relax/backend/contrib/mcblas/codegen.cc
 * \brief Implementation of the MCBLAS JSON serializer.
 */
...
class McblasJSONSerializer : public JSONSerializer {
 public:
  McblasJSONSerializer(ffi::Map<GenericConst, ffi::String> constant_names,
  ...
  NodeEntries VisitExpr_(const CallNode* call_node) final {
    ...
    inputs[0] = inputs_tmp[arg_idx["lhs"]->value.as<size_t>().value()];
    inputs[1] = inputs_tmp[arg_idx["rhs"]->value.as<size_t>().value()];
    if (inputs_tmp.size() == 3) {
      inputs[2] = inputs_tmp[arg_idx["bias"]->value.as<size_t>().value()];
    } else if (inputs_tmp.size() == 4) {
      inputs[2] = inputs_tmp[arg_idx["scaleA"]->value.as<size_t>().value()];
      inputs[3] = inputs_tmp[arg_idx["scaleB"]->value.as<size_t>().value()];
    }
...
ffi::Array<ffi::Module> McblasCompiler(ffi::Array<Function> functions,
  ...
  for (const auto& func : functions) {
    McblasJSONSerializer serializer(constant_names, AnalyzeVar2Value(func));
    serializer.serialize(func);
    auto graph_json = serializer.GetJSON();
    auto constant_names = serializer.GetConstantNames();
    const auto pf = tvm::ffi::Function::GetGlobalRequired("runtime.McblasJSONRuntimeCreate");
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("relax.ext.mcblas", McblasCompiler);
}
