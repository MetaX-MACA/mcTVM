...
/*!
 * \file src/relax/backend/contrib/mcdnn/codegen.cc
 * \brief Implementation of the mcDNN JSON serializer.
 */
...
class mcDNNJSONSerializer : public JSONSerializer {
 public:
  mcDNNJSONSerializer(Map<Constant, String> constant_names, Map<Var, Expr> bindings)
  ...
  NodeEntries VisitExpr_(const CallNode* call_node) final {
    ...
    if (composite_name.find("mcdnn.conv2d") != std::string::npos) {
...
Array<runtime::Module> mcDNNCompiler(Array<Function> functions, Map<String, ObjectRef> /*unused*/,
                                     Map<Constant, String> constant_names) {
  Array<runtime::Module> compiled_functions;

  for (const auto& func : functions) {
    mcDNNJSONSerializer serializer(constant_names, AnalyzeVar2Value(func));
    serializer.serialize(func);
    auto graph_json = serializer.GetJSON();
    auto constant_names = serializer.GetConstantNames();
    const auto* pf = runtime::Registry::Get("runtime.mcDNNJSONRuntimeCreate");
    ICHECK(pf != nullptr) << "Cannot find mcDNN runtime module create function.";
...
TVM_REGISTER_GLOBAL("relax.ext.mcdnn").set_body_typed(mcDNNCompiler);
