...
class WarpAccessRewriter : protected StmtExprMutator {
 public:
  explicit WarpAccessRewriter(int warp_size, arith::AnalyzerObj* analyzer, bool use_64bit_mask)
      : warp_size_(warp_size), analyzer_(analyzer), use_64bit_mask_(use_64bit_mask) {}
 ...
 protected:
  ...
  Expr VisitExpr_(const BufferLoadNode* op) override {
    ...
    PrimType mask_dtype = use_64bit_mask_ ? PrimType::UInt(64) : PrimType::UInt(32);
    PrimExpr mask = Call(mask_dtype, builtin::tvm_warp_activemask(), {}).as_or_throw<PrimExpr>();
...
class WarpMemoryRewriter : private StmtMutator {
 public:
  explicit WarpMemoryRewriter(int warp_size, bool use_64bit_mask)
      : warp_size_(warp_size), use_64bit_mask_(use_64bit_mask) {}
  ...
  Stmt VisitStmt_(const SeqStmtNode* op) {
    ...
    for (size_t i = 0; i < op->seq.size(); ++i) {
      ...
      if (alloc && alloc->buffer.scope() == "warp") {
        ...
        WarpAccessRewriter rewriter(warp_size_, analyzer_.get(), use_64bit_mask_);
...
Pass LowerWarpMemory() {
  auto pass_func = [](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    bool use_64bit_mask = target.value()->kind->name == "maca";
    WarpMemoryRewriter warp_memory_rewriter(warp_size, use_64bit_mask);
