...
/*!
 * \file src/runtime/contrib/mcblas/mcblas_json_runtime.cc
 * \brief A simple JSON runtime for MCBLAS.
 */
...
class McblasJSONRuntime : public JSONRuntimeBase {
 public:
  McblasJSONRuntime(const std::string& symbol_name, const std::string& graph_json,
  ...
  PackedFunc GetFunction(const String& name, const ObjectPtr<Object>& sptr_to_self) override {
    // JSONRuntimeBase::SetInputOutputBuffers(...) is not thread safe. Since McblasJSONRuntime
    // can be used by multiple GPUs running on different threads, we avoid using that function
    // and directly call mcBLAS on the inputs from TVMArgs.
  ...
  const char* type_key() const override { return "mcblas_json"; }  // May be overridden

  void Run(TVMArgs args) {
    auto* entry_ptr = tvm::contrib::McBlasLtThreadEntry::ThreadLocal();
    ...
    for (size_t i = 0; i < nodes_.size(); ++i) {
      ...
      if (node.GetOpType() == "kernel") {
        ...
        bool transa = false;
        bool transb = false;
        mcblasLtEpilogue_t epilogue = MCBLASLT_EPILOGUE_DEFAULT;
        ...
        auto [a_ptr, b_ptr, bias_ptr, scaleA_ptr, scaleB_ptr] =
            get_inputs(node, epilogue != MCBLASLT_EPILOGUE_DEFAULT, has_scale);
        ...
        tvm::contrib::CallMcblasLt(entry_ptr->handle, stream, entry_ptr->matmul_pref_desc, a_ptr,
...
runtime::Module McblasJSONRuntimeCreate(String symbol_name, String graph_json,
                                        const Array<String>& const_names) {
  auto n = make_object<McblasJSONRuntime>(symbol_name, graph_json, const_names);
  return runtime::Module(n);
}

TVM_REGISTER_GLOBAL("runtime.McblasJSONRuntimeCreate").set_body_typed(McblasJSONRuntimeCreate);

TVM_REGISTER_GLOBAL("runtime.module.loadbinary_mcblas_json")
    .set_body_typed(JSONRuntimeBase::LoadFromBinary<McblasJSONRuntime>);
