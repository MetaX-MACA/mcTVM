...
#include "./mcdnn_frontend/attention.h"
#endif
...
class mcDNNJSONRuntime : public JSONRuntimeBase {
 public:
  ...
  const char* type_key() const override { return "mcdnn_json"; }  // May be overridden
 ...
 private:
  ...
  std::function<void()> GetConv2DExec(const JSONGraphNode& node) {
    auto* entry_ptr = tvm::contrib::maca::McDNNThreadEntry::ThreadLocal();
    ...
    tvm::contrib::maca::FindAlgo(format, dims, groups, padding.data(), strides.data(),
    ...
    std::function<void()> op_exec = [=]() {
      auto stream = static_cast<mcStream_t>(GetMACAStream());
      ...
      if (has_bias) {
        tvm::contrib::maca::ConvolutionBiasActivationForward(
            mode, format, algo, dims, groups, act, coef, padding.data(), strides.data(),
            dilation.data(), a_ptr, b_ptr, out_ptr, bias_ptr, conv_dtype);
      } else {
        tvm::contrib::maca::ConvolutionForward(mode, format, algo, dims, groups, padding.data(),
                                               strides.data(), dilation.data(), a_ptr, b_ptr,
                                               out_ptr, conv_dtype);
      }
    };
    return op_exec;
  ...
  std::function<void()> GetAttentionExec(const JSONGraphNode& node) {
    ...
    auto runner = tvm::contrib::maca::McDNNSDPARunner::Create();
...
#else
    LOG(FATAL) << "Please build with MCDNN frontend to use attention op";
#endif
  }
...
runtime::Module mcDNNJSONRuntimeCreate(String symbol_name, String graph_json,
                                       const Array<String>& const_names) {
  auto n = make_object<mcDNNJSONRuntime>(symbol_name, graph_json, const_names);
  return runtime::Module(n);
}

TVM_REGISTER_GLOBAL("runtime.mcDNNJSONRuntimeCreate").set_body_typed(mcDNNJSONRuntimeCreate);

TVM_REGISTER_GLOBAL("runtime.module.loadbinary_mcdnn_json")
    .set_body_typed(JSONRuntimeBase::LoadFromBinary<mcDNNJSONRuntime>);
}  // namespace maca
}  // namespace contrib
}  // namespace runtime
}  // namespace tvm
