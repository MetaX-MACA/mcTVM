...
/*!
 * \file src/runtime/contrib/mcdnn/mcdnn_frontend/attention.cc
 * \brief mcDNN scale dot product attention implementation
 */
...
namespace contrib {
namespace maca {
void McDNNSDPARunnerNode::Init(int64_t batch, int64_t seq_len, int64_t num_heads,
                               int64_t num_kv_heads, int64_t head_size, int64_t head_size_v,
                               double scale, const DLDataType& data_type,
                               const std::string& layout) {
  graph_ = std::make_unique<mcdnn_frontend::graph::Graph>();

  CHECK(data_type.code == DLDataTypeCode::kDLFloat && data_type.bits == 16)
      << "Only float16 is supported";

  graph_->set_io_data_type(mcdnn_frontend::DataType_t::HALF)
      .set_intermediate_data_type(mcdnn_frontend::DataType_t::FLOAT)
      .set_compute_data_type(mcdnn_frontend::DataType_t::FLOAT);

  auto q_desc = mcdnn_frontend::graph::Tensor_attributes().set_name("Q").set_uid(kTensorIDQ);
  auto k_desc = mcdnn_frontend::graph::Tensor_attributes().set_name("K").set_uid(kTensorIDK);
  auto v_desc = mcdnn_frontend::graph::Tensor_attributes().set_name("V").set_uid(kTensorIDV);
  auto o_desc = mcdnn_frontend::graph::Tensor_attributes().set_name("Out").set_uid(kTensorIDOut);
  ...
  auto sdpa_options = mcdnn_frontend::graph::SDPA_attributes()
  ...
  MCDNN_FRONTEND_CALL(graph_->build(entry_ptr->handle, {mcdnn_frontend::HeurMode_t::A}));
}

void McDNNSDPARunnerNode::Run(const DLTensor* qkv, DLTensor* workspace, DLTensor* out) {
  MCDNN_CALL(
      mcdnnSetStream(McDNNThreadEntry::ThreadLocal()->handle, tvm::runtime::GetMCDAStream()));
  ...
  std::unordered_map<mcdnn_frontend::graph::Tensor_attributes::uid_t, void*> inputs = {
      {kTensorIDQ, q_ptr}, {kTensorIDK, k_ptr}, {kTensorIDV, v_ptr}, {kTensorIDOut, out_ptr}};

  McDNNThreadEntry* entry_ptr = McDNNThreadEntry::ThreadLocal();
  MCDNN_FRONTEND_CALL(graph_->execute(entry_ptr->handle, inputs, workspace->data));
}
}  // namespace maca
}  // namespace contrib
}  // namespace tvm
