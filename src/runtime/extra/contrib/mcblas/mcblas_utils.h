...
#define CHECK_MCBLAS_ERROR(fn)                                                                    \
  do {                                                                                            \
    ...
    TVM_FFI_ICHECK_EQ(error, MCBLAS_STATUS_SUCCESS) << "MCBLAS: " << GetMcblasErrorString(error); \
...
struct McBlasThreadEntry {
  ...
  static McBlasThreadEntry* ThreadLocal(DLDevice curr_device);
};  // McBlasThreadEntry
...
struct McBlasLtThreadEntry {
  ...
  static McBlasLtThreadEntry* ThreadLocal(DLDevice curr_device);
};  // McBlasLtThreadEntry
...
inline macaDataType_t GetMacaDataType(DLDataType type) {
  ...
  TVM_FFI_THROW(InternalError) << "Unsupported MACA type";
}
