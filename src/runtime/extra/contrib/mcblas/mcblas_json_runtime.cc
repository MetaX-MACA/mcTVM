...
class McblasJSONRuntime : public JSONRuntimeBase {
 public:
  ...
  ffi::Optional<ffi::Function> GetFunction(const ffi::String& name) override {
    ...
    // and directly call mcBLAS on the inputs from ffi::PackedArgs.
  ...
  const char* kind() const override { return "mcblas_json"; }  // May be overridden
  ...
  void Run(ffi::PackedArgs args) {
    ...
    auto* entry_ptr = tvm::contrib::McBlasLtThreadEntry::ThreadLocal(DLDevice{kDLMACA, device_id});
...
ffi::Module McblasJSONRuntimeCreate(ffi::String symbol_name, ffi::String graph_json,
                                    const ffi::Array<ffi::String>& const_names) {
  auto n = ffi::make_object<McblasJSONRuntime>(symbol_name, graph_json, const_names);
  return ffi::Module(n);
}
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def("runtime.McblasJSONRuntimeCreate", McblasJSONRuntimeCreate)
      .def("ffi.Module.load_from_bytes.mcblas_json",
           JSONRuntimeBase::LoadFromBytes<McblasJSONRuntime>);
}
