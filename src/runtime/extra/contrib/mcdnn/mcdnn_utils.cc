...
McDNNThreadEntry::McDNNThreadEntry() {
  auto func = tvm::ffi::Function::GetGlobalRequired("device_api.maca");
...
void SetConvDescriptors(McDNNThreadEntry* entry_ptr, int format, int dims, int groups,
  ...
  entry_ptr->conv_entry.data_type =
      McDNNDataType::DLTypeToMcDNNType(ffi::StringToDLDataType(conv_dtype));
  ...
  if (dims == 2) {
    ...
    TVM_FFI_ICHECK_EQ(format, 0)
        << "Use of layout MCDNN_TENSOR_NHWC is supported only for 4-D tensors.";
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("tvm.contrib.mcdnn.exists", []() -> bool {
    int device_id;
    MACA_CALL(mcGetDevice(&device_id));
    return McDNNThreadEntry::ThreadLocal(DLDevice{kDLMACA, device_id}, false)->exists();
  });
}
