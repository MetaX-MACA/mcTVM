...
/*!
 * \brief Replace copy from global to shared with memcpy async
 * \file inject_mxc_cp_async.cc
 */
...
uint32_t CountThreadIdxVar(const PrimExpr& expr) {
  if (const AddNode* add_node = expr.as<AddNode>()) {
    return CountThreadIdxVar(add_node->a) + CountThreadIdxVar(add_node->b);
  } else if (const SubNode* sub_node = expr.as<SubNode>()) {
    return CountThreadIdxVar(sub_node->a) + CountThreadIdxVar(sub_node->b);
  } else if (const VarNode* var_node = expr.as<VarNode>()) {
    if (var_node->name_hint == "threadIdx.x" || var_node->name_hint == "threadIdx.y" ||
        var_node->name_hint == "threadIdx.z") {
      return 1;
    }
    return 0;
  } else {
...
bool IsContinusAddress(const PrimExpr& expr) { return CountThreadIdxVar(expr) == 1; }

class MXCCopyAsyncInjector : public StmtMutator {
 public:
  ...
  Stmt InjectMXC(const BufferLoadNode* load, const BufferStoreNode* store, bool predicated = false,
    ...
    if (load->buffer.scope() == "global") {
      ...
      if (bytes == 4 || bytes == 8 || bytes == 16) {
        ...
        if (indices_lanes == 1) {
          ...
          return Evaluate(Call(store->buffer->dtype, tvm::tir::builtin::mxc_cp_async(), args));
        }
        ...
        if (!predicated) {
          ...
          if (src_offset.defined() && dst_offset.defined()) {
            return Evaluate(Call(store->buffer->dtype, tvm::tir::builtin::mxc_cp_async(),
          ...
          if (src_offset.defined() && dst_offset.defined()) {
            return Evaluate(
                Call(store->buffer->dtype, tvm::tir::builtin::mxc_cp_async(),
                     {store->buffer->data, mul(dst_offset, PrimExpr(index_factor)),
  ...
  Stmt VisitStmt_(const BufferStoreNode* store) {
    if (in_async && (store->buffer.scope() == "shared" || store->buffer.scope() == "shared.dyn") &&
        (store->indices.size() == 1 && IsContinusAddress(store->indices[0]))) {
      if (auto* load = store->value.as<BufferLoadNode>()) {
        return InjectMXC(load, store);
        // FIXME: if we want to write zero when predicate is false, we must set ret 0 if threads are
        // inactive, but when total threads are not multiiple of 64, for memcpy_async copy
        // continuous memory, the memory asscessed by inactive threads would be written zero, which
        // is not expected. } else if (auto* call = store->value.as<CallNode>()) {
        ...
        //       // Only default value of 0 is supported since 0 is the default value used by
        //       MemcpyAsync bool else_value_is_zero = false; if (auto* b =
        //       call->args[2].as<BroadcastNode>()) {
        ...
        //         return InjectMXC(load, store, true, call->args[0]);
        //       }
        //     }
        //   }
      }
...
Pass InjectMXCCopyAsync() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    Optional<Target> target = f->GetAttr<Target>(tvm::attr::kTarget);
    if (!target.defined()) {
      target = Optional<Target>(Target::Current(true));
    }
    if (target.defined() && target.value()->kind->name == "maca") {
      std::cout << "jcy" << std::endl;
      auto* n = f.CopyOnWrite();
      n->body = MXCCopyAsyncInjector()(n->body);
    }
    return f;
  };
  return CreatePrimFuncPass(pass_func, 0, "tir.InjectMXCCopyAsync", {});
}

TVM_REGISTER_GLOBAL("tir.transform.InjectMXCCopyAsync").set_body_typed(InjectMXCCopyAsync);
