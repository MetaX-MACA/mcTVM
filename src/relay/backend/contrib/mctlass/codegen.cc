...
/*!
 * \file src/relay/backend/contrib/mctlass/codegen.cc
...
class CodegenResultNode : public Object {
 public:
  ...
  static constexpr const char* _type_key = "contrib.mctlass.CodegenResult";
...
TVM_REGISTER_GLOBAL("contrib.mctlass.CodegenResult")
...
GenerateBodyOutput GenerateBody(const std::string& func_name, const std::string& ext_func_id,
  ...
  const auto* instantiate_template_func =
      runtime::Registry::Get("contrib.mctlass.instantiate_template");
...
/*! \brief Return the "mctlass" Target instance to use to guide compilation. */
Target GetCutlassTarget() {
  Target target = Target::Current(/*allow_not_defined=*/true);
  if (!target.defined() || target->kind->name != "mctlass") {
    // Use the default MCTLASS compilation options if no specific "mctlass" target was given
...
class CodegenCutlass : public backend::MemoizedExprTranslator<std::vector<Output>>,
 ...
 public:
  ...
  std::vector<Output> VisitExpr_(const CallNode* call) final {
    ...
    ICHECK(func) << "Only composite function is supported for MCTLASS.";
 ...
 private:
  ...
  GenerateBodyOutput GenerateBody(const CallNode* call, const std::string& func_name,
    ...
    return mctlass::GenerateBody(func_name, ext_func_id_, out_types_str, func_args, attrs,
                                 &buf_idx_);
  }
  ...
  /*! \brief The attrs of the external mctlass ext_func. */
  ...
  /*! \brief Statement of the function that will be compiled using MCTLASS kernels. */
 ...
 private:
  void GenCutlassFunc(const Function& function) {
    ...
    ICHECK(opt_global_symbol.defined())
        << "MCTLASS functions must have a " << tvm::attr::kGlobalSymbol << " attribute";
    ...
    VLOG(1) << "Creating mctlass C code for '" << sid << "' from:\n" << PrettyPrint(function);
  ...
  static const FunctionNode* GetCutlassFunctionNode(const Expr& expr) {
    if (const auto* function_node = expr.as<FunctionNode>()) {
      ...
      if (opt_compiler.defined() && opt_compiler.value() == "mctlass") {
...
/*!
 * \brief A small shim to redirect to the 'relay.ext.mctlass.compile_for_mctlass' Python
 * function which does the main MCTLASS training, c-code generation and compilation steps.
 */
tvm::transform::Pass CompileForCutlassImpl() {
  auto pass_func = [=](IRModule mod, const tvm::transform::PassContext& pass_ctx) {
    ...
    const auto* pf = runtime::Registry::Get("relay.ext.mctlass.compile_for_mctlass");
    ICHECK(pf != nullptr) << "Cannot find compile_for_mctlass function";
...
runtime::Module CreateCSourceModule(const IRModule& mod) {
  VLOG(1) << "Creating MCTLASS CSource module from:" << std::endl << PrettyPrint(mod);
  return CutlassModuleCodegen(mod).CreateCSourceModule();
}

}  // namespace

TVM_REGISTER_GLOBAL("relay.ext.mctlass.create_c_source_module").set_body_typed(CreateCSourceModule);

tvm::transform::Pass CompileForCutlass() {
  return transform::Sequential(
      {transform::OutlineCompilerFunctionsWithExistingGlobalSymbols("mctlass"),
       CompileForCutlassImpl(), transform::MarkCompilerFunctionsAsExtern("mctlass")});
}
